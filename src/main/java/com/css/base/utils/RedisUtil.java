...
//		redisTemplate.setStringSerializer();
	...
	public void deleteForHkey(String hkey) {
		if (opsForHash == null) {
			opsForHash = redisTemplate.opsForHash();
		}
		Set<String> keys = this.isHasFuzzyKey2(hkey);
...
//		Set<String> keys = jedis.keys(hkey);
     ...
     * @return 是否获取到
     */
	public boolean lock(String prefix,String lockkey){
	    return (Boolean) redisTemplate.execute((RedisCallback) connection -> {
	        String lock = prefix + lockkey;
            long expireAt = System.currentTimeMillis() + 600000 + 1;//锁过期时间
            ...
            if(acquire){
                ...
                if(Objects.nonNull(bytes) && bytes.length >0){
                    ...
                    if(expireTime < System.currentTimeMillis()){
                        //重新加锁，防止死锁
                        byte[] set = connection.getSet(lock.getBytes(), String.valueOf(System.currentTimeMillis() + 600000 + 1).getBytes());
